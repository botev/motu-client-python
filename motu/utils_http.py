...
from .utils_log import HTTPDebugProcessor, TRACE_LEVEL
# import urllib2
# import httplib
# import cookielib
# import utils_log
...
def open_url(url, **kwargs):
    ...
    if 'proxy' in kargs.keys():
        url_proxy = ''
        if 'user' in kargs['proxy'].keys():
            url_proxy = kargs['proxy']['user'] + ':' + kargs['proxy']['password'] + '@'

        url_proxy += kargs['proxy']['netloc']

        handlers.append(ProxyHandler({'http': url_proxy, 'https': url_proxy}))
        handlers.append(HTTPBasicAuthHandler())

        del kargs['proxy']

    if 'authentication' in kargs.keys():
        ...
        handlers.append(TLS1Handler(password_mgr))
    ...
    # create the request
    if data is not None:
        r = Request(url, data.encode("utf-8"), **kargs)
    else:
        r = Request(url, **kargs)
